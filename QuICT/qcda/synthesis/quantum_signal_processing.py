...
def check_polynomial_normalization(p:Polynomial, q:Polynomial, steps: int = 100, tolerance:float=0.001):
    """
    ...
    max_deviation = np.max(np.abs(summed_poly(x)-1))
...
def generate_A_tilt(polynomial_p:Polynomial):
    ...
    :return: A(y)
    """
    polynomial_p_conjugate = Polynomial(polynomial_p.coef.conjugate())
...
def generate_A_tilt_root(A_tilt:Polynomial, tolerance:float=0.001):
    """
...
"""def generate_poly_q(polynomial_p:Polynomial, k: int):
    ...
    unnormalized_A_tilt = polynomial_q
    for roots in A_tilt_roots:
        unnormalized_A_tilt = 
    for roots in A_tilt_roots:
        polynomial_q = polynomial_q*Polynomial([-1*roots, 0, 1])
    #Now we need to normalize polynomial_q
    #x = 2*np.random.rand()-1
    if k%2==0:
        polynomial_q = polynomial_q* Polynomial([0,1])
    normalization_constant = (1-polynomial_p(x)*Polynomial(np.conj(polynomial_p.coef))(x))/\
                             (Polynomial([1,0,-1])(x)*polynomial_q(x**2)*Polynomial(np.conj(polynomial_q.coef))(x**2))
    #polynomial_q = np.sqrt(normalization_constant)*polynomial_q
    return polynomial_q"""
...
def generate_phase_angle(polynomial_p:Polynomial, polynomial_q:Polynomial, k:int):
    ...
    assert check_polynomial_normalization(polynomial_p, polynomial_q), f"The polynomial p and polynomial q are not normalized"
...
class QuantumSignalProcessing:
    ...
    """
    def __init__(self, input_x, angle_sequence):
        self.input_x = input_x
        self.angle_sequence = angle_sequence
    def signal_matrix(self):
        WX = W_X_marix(self.input_x)
    ...
    def signal_circuit(self):
        signal_x = -2*np.arccos(self.input_x)
