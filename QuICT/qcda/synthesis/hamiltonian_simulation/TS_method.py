...
def prepare_eigenvector_gate(input_eigenvector):
    """
    Given 2*n size state vector,
    find gates act on [1,0....., 0{n}] such that produce this vectir
...
def int_reflection(binary_string):
    ...
    reflect the matrix elements|binary_string><binary_string|
...
def TS_method(coefficient_array, matrix_array, time, error, max_order):
    ...
    assert order < max_order,\
          f"The max order exceed {max_order}. \
            Adjust max allowed taylor expansion order."
    ########################################################################
    # steps for making composite gates
    # calculate the product of (sum_{k} H_{k})^k from 0 to order
    coefficient_array, control_hamilton_list = \
        product_gates(coefficient_array, matrix_array, order, time, time_steps)
    ...
    whole_circuit_reflection = gates_R(
        num_ancilla_qubit + matrix_dimension+1,
            num_ancilla_qubit + matrix_dimension+1)
