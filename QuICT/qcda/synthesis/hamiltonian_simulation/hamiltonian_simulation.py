...
class HamiltonianSimulation():
    def __init__(self, method):
        self.method = method
        assert self.method == "TS" or self.method == "Trotter" or self.method == "QSP", "Please select 'Trotter',\
                                                                                         'TS' or 'QSP' method."

    def execute(self, hamiltonian, time, initial_state, error=0.1, max_order=20):
        coefficient_array, hamiltonian_array = hamiltonian
        assert len(initial_state) == len(hamiltonian_array[0][0]), "The initial state size must \
                                                                    equal to hamiltonian row number."
        if self.method == "TS":
            TS_method_circuit, TS_method_gate_width, steps = TS_method(coefficient_array,
                                                                       hamiltonian_array,
                                                                       time,
                                                                       error,
                                                                       max_order,
