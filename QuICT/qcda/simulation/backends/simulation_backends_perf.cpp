...
inline std::vector<QuICT::GateBridgeEntry<precision_t>> get_qft_desc(
        uint64_t &qubit_num
) {
    using namespace std;
    fstream fs;
    ...
    auto vec = std::vector<QuICT::GateBridgeEntry<precision_t>>();
    fs >> qubit_num;
    string gate_name;
    while (fs >> gate_name) {
        ...
        if (gate_name == "h") {
            fs >> targ;
            vec.emplace_back(QuICT::GateBridgeEntry<precision_t>("h", {targ}, 0, nullptr));
        } else if (gate_name == "crz") {
            fs >> carg >> targ >> parg;
            vec.emplace_back(QuICT::GateBridgeEntry<precision_t>("crz", {carg, targ}, parg, nullptr));
        }
    }
    return vec;
}
...
inline void perf_simulator(
        ...
        uint64_t qubit_num,
        std::vector<QuICT::GateBridgeEntry<precision_t>> desc_vec,
...
TEST(SimPerf, HPerfTest) {
    ...
    auto desc_vec = vector<GateBridgeEntry<double>>();
    for (uint64_t i = 0; i < 500; ++i) {
        desc_vec.emplace_back(GateBridgeEntry<double>("h", {i % qubit_num}, 0, nullptr));
    }
    auto state = new complex<double>[1ULL << qubit_num];
...
TEST(SimPerf, CrzPerfTest) {
    ...
    auto desc_vec = vector<GateBridgeEntry<double>>();
    for (uint64_t i = 0; i < 500; ++i) {
        desc_vec.emplace_back(GateBridgeEntry<double>(
