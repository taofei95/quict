...
//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
// This simulator should be deprecated for its bad design.
// A new simulator could be found in `hybrid_simulator.h`.
//* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
...
namespace QuICT {
    ...
    template<typename Precision, SimulatorMode sim_mode = SimulatorMode::single>
    ...
    public:
        ...
        MonoTuneSimulator() {
            ...
            if constexpr(std::is_same<Precision, float>::value) {
                ss << "float ";
            } else if constexpr(std::is_same<Precision, double>::value) {
                ss << "double";
            }
            ss << "]";
            name_ = ss.str();
        ...
        void append_gate(
            ...
            gate_vec_.emplace_back(GateDescription<Precision>(
                    qasm_name, affect_args, parg, data_ptr
            ));
        }

        void append_gate(const GateDescription <Precision> &gate_desc) {
        ...
        void set_init_state(std::complex<Precision> *init_state) {
        ...
        inline void run(uint64_t qubit_num, std::complex<Precision> *init_state) {
            ...
            for (const auto &gate_desc: gate_vec_) {
                if (gate_desc.qasm_name_ == "h") {
                    apply_gate<HGate<Precision>>(gate_desc);
                } else if (gate_desc.qasm_name_ == "crz") {
                    apply_gate<CrzGate<Precision>>(gate_desc);
                } else if (gate_desc.qasm_name_ == "x") {
                    apply_gate<XGate<Precision>>(gate_desc);
                } else {
        ...
        inline void apply_gate(const GateDescription <Precision> &gate_desc) {
            ...
            using namespace std;
            if constexpr(is_same<HGate<Precision>, Gate>::value) {
                // H gate
                auto gate = HGate<Precision>(gate_desc.affect_args_[0]);
                apply_gate(gate);
            } else if constexpr(is_same<CrzGate<Precision>, Gate>::value) {
                // Crz gate
                auto gate = CrzGate<Precision>(
                        gate_desc.affect_args_[0],
                        gate_desc.affect_args_[1],
                        gate_desc.parg_);
                apply_gate(gate);
            } else if constexpr(is_same<XGate<Precision>, Gate>::value) {
                // X gate
    ...
    protected:
        ...
        template<class Gate>
        inline void apply_gate(const Gate &gate) {
            if constexpr(sim_mode == SimulatorMode::single) {
                uint64_t task_num = 1ULL << (qubit_num_ - gate_qubit_num<Gate>::value);


#pragma omp parallel for default(none) shared(task_num, gate)
...
#pragma omp parallel for default(none) shared(task_num, gate)
                for (uint64_t task_id = 0; task_id < task_num; task_id += batch_size) {
                    apply_gate_batch_task<batch_size, Gate>(task_id, gate);
                }
                ...
                if constexpr(std::is_same<HGate<Precision>, Gate>::value) {
                    constexpr uint64_t batch_size = 4;
...
#pragma omp parallel for default(none) shared(task_num, gate)
                ...
                } else if constexpr(std::is_same<CrzGate<Precision>, Gate>::value) {
                    constexpr uint64_t batch_size = 4;
...
#pragma omp parallel for default(none) shared(task_num, gate)
        ...
        inline void apply_gate_single_task(
            ...
            auto tmp_arr_1 = marray_t<Precision, 2>();
        ...
        inline void apply_gate_single_task(
                const uint64_t task_id,
                const UnitaryGateN <N, Precision> &gate
        ) {
            if constexpr(N == 1) {
                auto ind = index(task_id, qubit_num_, gate.targ_);
                auto tmp_arr_1 = marray_t<Precision, 2>();
                ...
                marray_t<Precision, vec_slice_sz> tmp_arr;
#pragma unroll
                for (size_t i = 0; i < vec_slice_sz; ++i) {
                    tmp_arr[i] = state_vector_[ind[i]];
                    state_vector_[ind[i]] = static_cast<mat_entry_t<Precision>>(0);
                }
#pragma unroll
                for (size_t i = 0; i < vec_slice_sz; ++i) {
#pragma unroll
        ...
        template<uint64_t batch_size, class Gate>
...
//                _mm256_zeroupper();
    ...
    protected:
        std::complex<Precision> *state_vector_ = nullptr;
//        Precision *state_vec_sep_re_ = nullptr, *state_vec_sep_im_ = nullptr;
        uint64_t qubit_num_ = 0;
        std::vector<GateDescription < Precision>> gate_vec_;
        std::string name_;
    };
}
