...
class MCTS(object):
    ...
    def __init__(self, play_times: int = 1, gamma: float = 0.8, Gsim: int = 30, num_of_swap_gates: int = 15, num_of_process: int = 4, bp_mode: int = 0, 
                Nsim: int = 2, selection_times: int = 5000, c = 20, graph_name: str = None, coupling_graph: CouplingGraph = None, virtual_loss: float = 0,  is_generate_data: bool = False, 
                threshold_size: int = 150, experience_pool: ExperiencePool = None, extended: bool = True, with_predictor: bool = False, info: int = 0, 
        ...
        self._mcts_wrapper = MCTSTreeWrapper(major = major, method = method, info =info, extended = extended, with_predictor = with_predictor, is_generate_data = is_generate_data, threshold_size = threshold_size, gamma = self._gamma, c = self._c, virtual_loss = virtual_loss, 
                                        bp_mode = bp_mode, num_of_process = num_of_process, size_of_subcircuits = self._Gsim, num_of_iterations = self._selection_times, 
                                        num_of_swap_gates = num_of_swap_gates, num_of_playout = self._Nsim, num_of_edges = self._coupling_graph.num_of_edge, 
                                        coupling_graph = self._coupling_graph.adj_matrix, distance_matrix = self._coupling_graph.distance_matrix, 
                                        edge_label = self._coupling_graph.label_matrix, feature_matrix = self._coupling_graph.node_feature) 
