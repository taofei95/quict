...
class RLMCTS(MCTS):
    def __init__(self, play_times: int = 1, gamma: float = 0.7, Gsim: int = 30, num_of_swap_gates: int = 10, num_of_process: int = 2, bp_mode: int = 0, extended: bool = False, 
                Nsim: int = 500, selection_times: int = 40, c = 20, graph_name: str = None, virtual_loss: float = 10,  is_generate_data: bool = False,  with_predictor: bool = False,
                threshold_size: int = 150, experience_pool: ExperiencePool = None, model_file_path: str = None, device: torch.device = torch.device("cuda"), device_id: int = 0,**params):
        """
        Params:
        ...
        self._mcts_wrapper = RLMCTSTreeWrapper(is_generate_data = is_generate_data, threshold_size = threshold_size,
                                        ...
                                        num_of_edges = self._coupling_graph.num_of_edge, coupling_graph = self._coupling_graph.adj_matrix, 
                                        distance_matrix = self._coupling_graph.distance_matrix, edge_label = self._coupling_graph.label_matrix, 
                                        feature_matrix = self._coupling_graph.node_feature, model_file_path = model_file_path.encode("utf-8"),
                                        device = d) 
    ...
    def search(self, logical_circuit: Circuit, init_mapping: List[int], extended: bool = False):
        ...
        print(front_layer_)
        print(qubit_mask_)
        self._mcts_wrapper.load_data(num_of_logical_qubits = logical_circuit.circuit_width(), num_of_gates = self._circuit_dag.size, circuit = self._circuit_dag.node_qubits, dependency_graph = self._circuit_dag.compact_dag,
