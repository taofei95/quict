...
class DataGenerator {
 public:
  DataGenerator(int major, int info, bool with_predictor, bool is_generate_data,
                ...
                int feature_dim, int num_of_edges, bool extended,
  ...
  void load_data(int num_of_gates, int num_of_logical_qubits, int* circuit,
                 int* dependency_graph, std::vector<int> qubit_mapping,
                 std::vector<int> qubit_mask, std::vector<int> front_layer);
  void run();
  void clear();
  void update_model();
