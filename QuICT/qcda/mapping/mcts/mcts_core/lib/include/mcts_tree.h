...
float simulate_thread(mcts::MCTSNode cur_node, int seed, int num_of_sawp_gates,
                      float gamma);
float simulate_thread_max(mcts::MCTSNode cur_node, int seed,
                          int num_of_sawp_gates, float gamma);
float simulate_thread_feng(mcts::MCTSNode cur_node, int seed,
...
class MCTSTree {
 public:
  ...
  MCTSTree(int major, int method, int info, bool extened, bool with_predictor,
           ...
           int num_of_iterations, int num_of_playout, int num_of_qubits,
           int feature_dim, int num_of_edges, int* coupling_graph,
           int* distance_matrix, int* edge_label, float* feature_matrix);

  void load_data(int num_of_gates, int num_of_logical_qubits, int* circuit,
                 int* dependency_graph);
  void build_search_tree(std::vector<int>& qubit_mapping,
