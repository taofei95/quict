...
class Sample {
 public:
  ...
  Sample(int cid, int pid, torch::Tensor& qubits, torch::Tensor& padding_mask,
         torch::Tensor& adj) {
    this->cid = cid;
    this->pid = pid;
    this->qubits = qubits;
...
void inference_thread(SampleQueue& sample_queue, std::atomic_flag* thread_flag,
                      int num_of_process, int batch_size,
                      const std::string model_file_path, int device,
                      std::atomic_flag* res_flags, float* res_values,
