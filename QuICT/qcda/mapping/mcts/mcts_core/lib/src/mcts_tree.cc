...
int nearest_neighbour_cost(mcts::MCTSNode& cur_node, 
            std::vector<int>& gates, 
            std::vector<int>& cur_mapping){
    int res = 0;
    for(auto& g : gates){
        std::vector<int> qubits = cur_node.circuit->get_gate_qubits(g);
        res += cur_node.coupling_graph->distance(cur_mapping[qubits[0]], cur_mapping[qubits[1]]);
    }
    return res; 
}
...
int swap_cost(mcts::MCTSNode& cur_node, mcts::Gate& swap_gate){
    ...
    std::vector<int> l_qubits = {cur_node.inverse_qubit_mapping[swap_gate.ctrl], cur_node.inverse_qubit_mapping[swap_gate.tar]};
...
float mcts::simulate_thread(mcts::MCTSNode cur_node, int seed, int num_of_swap_gates, float gamma){
    float res = 0, weight = 1.0;
    int num_of_executed_gates = 0, num_of_added_swap_gates = 0;
    while(num_of_added_swap_gates < num_of_swap_gates && !cur_node.is_terminal_node()){
        cur_node.update_candidate_swap_list();
        std::vector<float> heuristic_prob(cur_node.num_of_children, 0.0);
        int nnc_base = nearest_neighbour_cost(cur_node, cur_node.front_layer, cur_node.qubit_mapping);
        for(int i = 0; i < cur_node.num_of_children; i++){
            heuristic_prob[i] = f(static_cast<float>(nnc_base - swap_cost(cur_node, cur_node.candidate_swap_list[i])));
        }
...
float mcts::simulate_thread_max(mcts::MCTSNode cur_node, int seed, int num_of_swap_gates, float gamma){
float res = 0, weight = 1.0;
    int num_of_executed_gates = 0, num_of_added_swap_gates = 0;
    while(num_of_added_swap_gates < num_of_swap_gates && !cur_node.is_terminal_node()){
        cur_node.update_candidate_swap_list();
...
float mcts::simulate_thread_feng(mcts::MCTSNode cur_node, int seed, int size_of_subcircuits, float gamma){
    float res = 0, weight = 1.0;
    ...
    if(num_of_executed_gates > size_of_subcircuits){
        res = std::pow(gamma, num_of_added_swap_gates /2) * (float)(size_of_subcircuits);
    }else{
        res = std::pow(gamma, num_of_added_swap_gates /2) * (float)(num_of_executed_gates);
...
mcts::MCTSTree::MCTSTree(int major,int method, int info, bool extended, bool with_predictor, bool is_generate_data, int threshold_size, 
    ...
    long capacity = static_cast<long>(this->num_of_iterations) * static_cast<long>(num_of_edges) * 500;
...
void mcts::MCTSTree::load_data(int num_of_gates, int num_of_logical_qubits, 
    ...
    this->circuit = std::move(mcts::Circuit(num_of_gates, circuit, dependency_graph));
    this->adj_list.clear();
...
void mcts::MCTSTree::build_search_tree(std::vector<int>& qubit_mapping, std::vector<int>& qubit_mask, std::vector<int>& front_layer){
    ...
    *(this->root_node) =  std::move(mcts::MCTSNode(&(this->coupling_graph),
                                        &(this->circuit),
                                        qubit_mapping,
                                        qubit_mask,
                                        front_layer,
    ...
    while(!cur_node->is_terminal_node()){
        ...
        if(this->info == 1){
            ...
            std::cout<<" swap gate:   "<<this->num_of_added_swap_gates
                    ...
                    <<" num of remaining gates:  "<<cur_node->num_of_gates<<std::endl;
            std::cout<<"------------------------------------------"<<std::endl; 

            std::cout<<"memory used: "<<this->mcts_node_pool.getSize()<<std::endl;
        }
    }
...
float mcts::MCTSTree::upper_condfidence_bound(mcts::MCTSNode* cur_node){
    return this->gamma * cur_node->value + cur_node->reward + this->c * sqrt(log((float)(cur_node->parent->visit_count)) / (cur_node->visit_count+ 0.001)); 
}

float mcts::MCTSTree::upper_condfidence_bound_with_predictor(mcts::MCTSNode* cur_node){
    return this->gamma * cur_node->value + cur_node->reward + this->c * cur_node->prob * sqrt((float)(cur_node->parent->visit_count)/ (1.0 + cur_node->visit_count));
}
...
float mcts::MCTSTree::rollout(MCTSNode* cur_node){
    ...
    for(int i = 0; i < this->num_of_playout; i++){
        if(this->method == 0)
            res[i] = simulate_thread(*cur_node, i, this->num_of_swap_gates, this->gamma);
        else if(this->method == 1)
            res[i] = simulate_thread_feng(*cur_node, i, this->size_of_subcircuits, this->gamma);
        else if(this->method == 2)
            res[i] = simulate_thread_max(*cur_node, i, this->num_of_swap_gates, this->gamma);
        else   
...
mcts::MCTSNode* mcts::MCTSTree::create_child_node_by_swap_gate(mcts::MCTSNode* cur_node, int i){
    ...
    *child_node = std::move(mcts::MCTSNode(&(this->coupling_graph),
                                   &(this->circuit),
                                   qubit_mapping,
                                   qubit_mask,
                                   cur_node->front_layer,
                                   swap_gate,
                                   cur_node,
...
void mcts::MCTSTree::fallback(mcts::MCTSNode* cur_node){
    ...
    while(this->fallback_count>0){
        this->num_of_added_swap_gates -= 1; 
        mcts::Gate swap_gate = this->coupling_graph.edges[this->swap_label_list.back()];
...
void mcts::MCTSTree::generate_data(mcts::MCTSNode* cur_node, int swap_gate_label){
    std::vector<int> subcircuit = cur_node->get_subcircuit(this->threshold_size);
    this->adj_list.emplace_back(std::move(this->circuit.get_adj_matrix(subcircuit, this->threshold_size)));
    this->qubits_list.emplace_back(std::move(this->circuit.get_qubits_matrix(subcircuit, cur_node->qubit_mapping, this->threshold_size)));
...
int mcts::MCTSTree::get_num_of_samples(){
...
int* mcts::MCTSTree::get_adj_list(){
    ...
    for(int i = 0; i < this->num_of_samples; i++){
        memcpy(static_cast<void*>(this->adj_mem + i * n), 
            static_cast<void*>(this->adj_list[i].data()),
            sizeof(int) * n);
    }
    return this->adj_mem;
}
int* mcts::MCTSTree::get_qubits_list(){
    ...
    for(int i = 0; i < this->num_of_samples; i++){
        memcpy(static_cast<void*>(this->qubits_mem + i * n), 
            static_cast<void*>(this->qubits_list[i].data()),
            sizeof(int) * n);
    }
