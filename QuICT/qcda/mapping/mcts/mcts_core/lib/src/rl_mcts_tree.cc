...
void mcts::inference_thread(mcts::SampleQueue& sample_queue,
                            std::atomic_flag* thread_flag, int num_of_process,
                            int batch_size, std::string model_file_path,
                            int device, std::atomic_flag* res_flags,
  ...
  torch::Tensor adj_batch[batch_size], qubits_batch[batch_size],
      padding_mask_batch[batch_size];
  int ids[batch_size];
  int batch_count;
  while (thread_flag->test_and_set(std::memory_order_acquire)) {
    ...
    if (batch_count > 0) {
      ...
      inputs[0] = std::move(torch::stack(at::ArrayRef<torch::Tensor>(
                                             qubits_batch, batch_count),
                                         0))
                      .to(device_type, torch::kInt32);
      inputs[1] = std::move(torch::stack(at::ArrayRef<torch::Tensor>(
                                             padding_mask_batch, batch_count),
                                         0))
                      .to(device_type);
      inputs[2] =
          std::move(torch::stack(
                        at::ArrayRef<torch::Tensor>(adj_batch, batch_count), 0))
              .to(device_type, torch::kInt32);

      auto outputs = nn_model.forward(inputs).toTuple();
      auto values = outputs->elements()[1].toTensor();
      auto policy =
          outputs->elements()[0].toTensor().to(at::Device(torch::kCPU));
...
mcts::RLMCTSTree::RLMCTSTree(
    int major, int info, bool extened, bool with_predictor,
    bool is_generate_data, int threshold_size, float gamma, float c,
    float virtual_loss, int bp_mode, int num_of_process,
    int size_of_subcircuits, int num_of_swap_gates, int num_of_iterations,
    int num_of_playout, int num_of_qubits, int feature_dim, int num_of_edges,
    int* coupling_graph, int* distance_matrix, int* edge_label,
    float* feature_matrix, const char* model_file_path, int device)
    : mcts::MCTSTree(major, 0, info, extened, with_predictor, is_generate_data,
                     threshold_size, gamma, c, virtual_loss, bp_mode,
                     num_of_process, size_of_subcircuits, num_of_swap_gates,
                     num_of_iterations, num_of_playout, num_of_qubits,
                     feature_dim, num_of_edges, coupling_graph, distance_matrix,
  ...
  this->inferencer = std::move(std::thread(
      inference_thread, std::ref(*(this->sample_queue)), this->thread_flag,
...
mcts::RLMCTSTree::RLMCTSTree(
    ...
    float virtual_loss, int bp_mode, int num_of_process,
    int size_of_subcircuits, int num_of_swap_gates, int num_of_iterations,
    int num_of_playout, mcts::CouplingGraph& coupling_graph, int cid,
  ...
  long capacity = static_cast<long>(this->num_of_iterations) *
                  static_cast<long>(coupling_graph.num_of_edges) * 50;
...
mcts::RLMCTSTree::~RLMCTSTree() {
  if (mode == 0) {
    ...
    delete[] this->res_flags, this->res_values, this->res_probs;
    delete this->thread_flag;
  }
}

void mcts::RLMCTSTree::load_data(int num_of_logical_qubits,
...
float mcts::RLMCTSTree::evaluate(mcts::MCTSNode* cur_node, int pid) {
  ...
  torch::Tensor adj_tensor = torch::empty(
      {this->threshold_size, 5}, torch::TensorOptions().dtype(torch::kInt32));
  torch::Tensor qubits_tensor = torch::empty(
      {this->threshold_size, 2}, torch::TensorOptions().dtype(torch::kInt32));
  torch::Tensor padding_mask_tensor = torch::ones(
      {this->threshold_size}, torch::TensorOptions().dtype(torch::kBool));
  ...
  memcpy(qubits_tensor.data_ptr(),
         static_cast<void*>(this->circuit
                                .get_qubits_matrix(subcircuit,
                                                   cur_node->qubit_mapping,
                                                   this->threshold_size)
                                .data()),
         sizeof(int) * this->threshold_size * 2);

  this->sample_queue->enqueue(std::move(
      mcts::Sample(this->cid, pid, std::move(qubits_tensor),
                   std::move(padding_mask_tensor), std::move(adj_tensor))));
  ...
  if (this->extended) {
    ...
    for (int i = 0; i < cur_node->num_of_children; i++) {
      ...
      cur_node->probs_of_children[i] =
          prob_accessor[this->coupling_graph.swap_gate_label(
              cur_node->candidate_swap_list[i])];
    }
  }
