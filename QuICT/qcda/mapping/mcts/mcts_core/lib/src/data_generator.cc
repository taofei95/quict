...
mcts::DataGenerator::DataGenerator(
    int major, int info, bool with_predictor, bool is_generate_data,
    int threshold_of_circuit, float gamma, float c, float virtual_loss,
    int bp_mode, int num_of_process, int size_of_subcircuits,
    int num_of_swap_gates, int num_of_iterations, int num_of_playout,
    ...
    int num_of_circuit_process, int inference_batch_size, int max_batch_size) {
  this->major = major;
  this->info = info;
  ...
  this->res_flags =
      new std::atomic_flag[this->num_of_process * this->num_of_circuit_process];
  this->res_values =
      new float[this->num_of_process * this->num_of_circuit_process];
  this->res_probs =
      new torch::Tensor[this->num_of_process * this->num_of_circuit_process];
  ...
  this->inferencer = std::move(std::thread(
      mcts::inference_thread, std::ref(this->sample_queue), this->thread_flag,
      this->num_of_process, this->inference_batch_size, this->model_file_path,
      this->device, this->res_flags, this->res_values, this->res_probs));
}

mcts::DataGenerator::~DataGenerator() {
  this->thread_flag->clear();
  this->inferencer.join();
  delete[] this->res_flags, this->res_values, this->res_probs, this->adj_mem,
      this->qubits_mem, this->num_mem, this->swap_label_mem, this->value_mem,
      this->qubits_mem;
...
void mcts::DataGenerator::load_data(int num_of_gates, int num_of_logical_qubits,
  ...
  this->circuit_list.emplace_back(
...
#pragma omp for
    for (int i = 0; i < this->num_of_circuit; i++) {
      mcts::RLMCTSTree rl_mcts(
          this->major, this->info, this->extended, this->with_predictor,
          this->is_generate_data, this->threshold_size, this->gamma, this->c,
          this->virtual_loss, this->bp_mode, this->num_of_process,
          this->size_of_subcircuits, this->num_of_swap_gates,
          this->num_of_iterations, this->num_of_playout, this->coupling_graph,
          cid, &(this->sample_queue), this->res_flags, this->res_values,
          this->res_probs);
      ...
      this->adj_list.insert(this->adj_list.end(), rl_mcts.adj_list.begin(),
                            rl_mcts.adj_list.end());
      this->qubits_list.insert(this->qubits_list.end(),
                               rl_mcts.qubits_list.begin(),
                               rl_mcts.qubits_list.end());
      this->num_list.insert(this->num_list.end(), rl_mcts.num_list.begin(),
                            rl_mcts.num_list.end());
      this->swap_label_list.insert(this->swap_label_list.end(),
                                   rl_mcts.swap_label_list_n.begin(),
                                   rl_mcts.swap_label_list_n.end());
      this->value_list.insert(this->value_list.end(),
                              rl_mcts.value_list.begin(),
                              rl_mcts.value_list.end());
      this->action_prob_list.insert(this->action_prob_list.end(),
                                    rl_mcts.action_prob_list.begin(),
                                    rl_mcts.action_prob_list.end());
...
float* mcts::DataGenerator::get_action_prob_list(
    std::vector<int>& batch_samples) {
