...
mcts::CouplingGraph::CouplingGraph(int num_of_qubits, int feature_dim,
                                   int num_of_edges, int* adj_matrix,
...
std::vector<int> mcts::CouplingGraph::shortest_path(int s, int t) {
  ...
  std::vector<int> parent(this->num_of_qubits, -1),
      mark(this->num_of_qubits, 1), dis(this->num_of_qubits, 0);
  std::queue<int> q;
  q.push(s);
  mark[s] = 0;
  while (!q.empty()) {
...
mcts::Circuit::Circuit() { this->num_of_gates = 0; }
...
std::vector<int> mcts::Circuit::get_adj_matrix(std::vector<int>& gates,
                                               int length) {
...
std::vector<int> mcts::Circuit::get_qubits_matrix(
    std::vector<int>& gates, std::vector<int>& qubit_mapping, int length) {
  assert(gates.size() <= length);
