...
mcts::MCTSNode::MCTSNode(CouplingGraph* coupling_graph, Circuit* circuit,
  ...
  this->inverse_qubit_mapping =
      std::move(std::vector<int>(num_of_physical_qubits, -1));
...
std::vector<int> mcts::MCTSNode::get_subcircuit(int num_of_circuits) {
  ...
  std::vector<int> res(n, 0),
      logical_qubit_mask(this->qubit_mapping.size(), -1);
  std::queue<int> q;
...
bool mcts::MCTSNode::is_gate_free(int gate) {
  std::vector<int> qubits = this->circuit->get_gate_qubits(gate);
  int ctrl = this->qubit_mask[this->qubit_mapping[qubits[0]]],
      targ = this->qubit_mask[this->qubit_mapping[qubits[1]]];
  return (ctrl == -1 || ctrl == gate) && (targ == -1 || targ == gate);
}

bool mcts::MCTSNode::is_gate_free(int gate,
                                  std::vector<int>& logical_qubit_mask) {
  std::vector<int> qubits = this->circuit->get_gate_qubits(gate);
  int ctrl = logical_qubit_mask[qubits[0]],
      targ = logical_qubit_mask[qubits[1]];
  return (ctrl == -1 || ctrl == gate) && (targ == -1 || targ == gate);
}
...
void mcts::MCTSNode::update_candidate_swap_list() {
  ...
  this->probs_of_children = std::move(std::vector<float>(
      num_of_swap_gates, 1.0 / (num_of_swap_gates + 0.00001)));
...
void mcts::MCTSNode::update_candidate_swap_list_extended() {
  ...
  this->probs_of_children = std::move(
      std::vector<float>(this->coupling_graph->num_of_edges,
                         1 / (float)(this->coupling_graph->num_of_edges)));
...
int mcts::MCTSNode::update_by_swap_gate(mcts::Gate& swap_gate) {
  ...
  std::vector<int> l_qubits = {this->inverse_qubit_mapping[swap_gate.ctrl],
                               this->inverse_qubit_mapping[swap_gate.tar]};
