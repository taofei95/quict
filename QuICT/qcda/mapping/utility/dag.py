...
class DAG(object):
    ...
    def get_subcircuit(self, front_layer: List[int], qubit_mask: List[int], num_of_gates: int, gates_threshold: int  = -1)->Tuple[int, np.ndarray]:
        """
        ...
        while index < num_of_gates and len(front_layer_stack) > 0:
            ...
            if index >= num_of_gates:
                ...
                for s in subcircuit:
                    print(list(s))
                raise Exception("The number of gates in the subcircuit %d is greater than the given number %d"%(index, num_of_gates))
