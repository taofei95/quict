...
class RandomCircuitGenerator(object):
    def __init__(self, minimum: int = 50, maximum: int = 2000, min_num_of_qubits: int = 5, max_num_of_qubits: int = 20,
                 seed: int = 0):
    ...
    def _is_valid_qubits(self, qubits: Tuple[int, int], pre_gate_qubits: Tuple[int, int]) -> bool:
        ...
        if (not len(pre_gate_qubits) == 0) and (
                qubits[0] == pre_gate_qubits[0] and qubits[1] == pre_gate_qubits[1]) or (
                qubits[1] == pre_gate_qubits[0] and qubits[0] == pre_gate_qubits[1]):
            return False
        return True
