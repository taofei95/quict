...
class Evaluator(object):
    def __init__(self, coupling_graph: str = None, config: GNNConfig = None,  max_gates: int = 200, min_gates: int = 50, max_qubits: int = 20, min_qubits: int = 5, num_of_circuits: int = 5, benchmark: Benchmark = Benchmark.RANDOM):
    ...
    def __call__(self, model_path: str = None, rl_mode: MCTSMode = MCTSMode.SEARCH, mode: EvaluateMode = EvaluateMode.SEARCH, sim: SimMode = SimMode.MAX, output_path: str = None):
        ...
        with open(f"{output_path}/evaluate_res",'w') as f:
            for i, circuit in enumerate(self._circuits):
                if mode == EvaluateMode.SEARCH:
                    ...
                    f.write("%d %d %d \n"%(circuit.circuit_size(), num_of_swap_gates, num_of_swap_gates_random))
