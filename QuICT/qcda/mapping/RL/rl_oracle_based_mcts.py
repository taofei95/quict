...
class RLOracleBasedMCTS(TableBasedMCTS):
    def _upper_confidence_bound(self, node: MCTSNode)->float:
        ...
        return float(node.sim_gates + node.reward)/float(node.sim_value + 1)  + self._c* np.sqrt(np.log(node.parent.visit_count) / (node.visit_count+0.001))
    def _upper_confidence_bound_with_predictor(self, node: MCTSNode)->float:
        """
        The upper confidence bound with predictor of the node
        """
        return   float(node.sim_gates + node.reward)/float(node.sim_value + 1)  + self._c* node.prob_of_edge *np.sqrt(node.parent.visit_count) / (node.visit_count+1) 
    ...
    def search(self, logical_circuit: Circuit = None, init_mapping: List[int] = None):
        ...
        self._root_node.sim_gates = min(self._size_threshold, self.root_node.num_of_gates)
        ...
        while self._root_node.is_terminal_node() is not True:
            ...
            print(self._root_node.sim_gates)
    ...
    def _evaluate(self, node: MCTSNode):
        ...
        node.sim_value = node.sim_gates / value
        node.value =   value
        node.w =  node.sim_gates / value 
    ...
    def _decide(self,node : MCTSNode)-> MCTSNode:
        ...
        #node.clear()
        if node.sim_gates<=0:
            node.sim_gates = min(self._size_threshold, node.num_of_gates)
        return node
    ...
    def _expand(self, node: MCTSNode):
        ...
        if node.sim_gates>0:
            for swap, prob in zip(node.candidate_swap_list, node.edge_prob):
                ...
                child_node.sim_value = child_node.num_of_gates
    ...
    def _backpropagate(self, node : MCTSNode, value: int):
        ...
        cur_node = node
        bp_value = value + 1
        if self._sim == SimMode.MAX: 
            while cur_node.parent is not None:
                if bp_value < cur_node.parent.value:
                    cur_node.parent.sim_value = bp_value
                    cur_node.parent.value =  cur_node.parent.sim_gates /  cur_node.parent.sim_value
                bp_value = bp_value + 1
                cur_node = cur_node.parent

        elif self._sim == SimMode.AVERAGE:
            while cur_node.parent is not None:
                cur_node.parent.w += bp_value
                cur_node.parent.sim_value = cur_node.parent.w / cur_node.visit_count
                cur_node.parent.value = cur_node.parent.sim_gates /  cur_node.parent.sim_value
                bp_value = bp_value + 1
                cur_node = cur_node.parent
    ...
    def _get_data(self, node: MCTSNode):
        ...
        #print(qubit_mask)
        num_of_gates, state = self._circuit_dag.get_subcircuit(front_layer = node.front_layer, qubit_mask = qubit_mask, num_of_gates = node.sim_gates, gates_threshold = self._size_threshold)
    ...
    def random_simulate(self, logical_circuit: Circuit, init_mapping: List[int]):    
        ...
        cur_node = MCTSNode(circuit_dag = self._circuit_dag , coupling_graph = self._coupling_graph,
                                  front_layer = self._circuit_dag.front_layer, qubit_mask = qubit_mask.copy(), cur_mapping = init_mapping.copy(), parent =None)
        cur_node.sim_gates = self._size_threshold
