...
from QuICT.qcda.mapping.ai.swap_pred_layered import SwapPredMix
...
class Trainer:
    def __init__(
        ...
        t_dataset, v_dataset = dataset.split_tv(
            point=90
        ...
        self.model = SwapPredMix(
            max_qubit=50,
            hetero_metadata=t_dataset.hetero_metadata,
            gcn_out_channel=200,
            out_channel=1,
        ).to(device)
        ...
        self.t_loader = t_dataset.loader(
            batch_size=batch_size, shuffle=shuffle, device=device
        )
        self.v_loader = v_dataset.loader(
            batch_size=batch_size, shuffle=shuffle, device=device
        )
