...
class MCTS(object):
    ...
    def __init__(
        ...
        bp_mode: int = 0,
        Nsim: int = 2,
        selection_times: int = 5000,
        c=20,
        graph_name: str = None,
        coupling_graph: CouplingGraph = None,
        virtual_loss: float = 0,
        is_generate_data: bool = False,
        threshold_size: int = 150,
        experience_pool: ExperiencePool = None,
        extended: bool = True,
        ...
        **params
        ...
        self._mcts_wrapper = MCTSTreeWrapper(
            major=major,
            method=method,
            info=info,
            extended=extended,
            with_predictor=with_predictor,
            is_generate_data=is_generate_data,
            ...
            size_of_subcircuits=self._Gsim,
            num_of_iterations=self._selection_times,
            num_of_swap_gates=num_of_swap_gates,
            num_of_playout=self._Nsim,
    ...
    def search(self, logical_circuit: Circuit, init_mapping: List[int]):
        ...
        self._logical_circuit_dag = DAG(
            circuit=logical_circuit, mode=Mode.WHOLE_CIRCUIT
        )
        ...
        front_layer_ = [
            self._circuit_dag.index[i] for i in self._circuit_dag.front_layer
        ]
        qubit_mask_ = [
            self._circuit_dag.index[i] if i != -1 else -1 for i in qubit_mask
        ]
        # print(front_layer_)
        # print(qubit_mask_)
        self._mcts_wrapper.load_data(
            num_of_logical_qubits=logical_circuit.width(),
            num_of_gates=self._circuit_dag.size,
            circuit=self._circuit_dag.node_qubits,
            dependency_graph=self._circuit_dag.compact_dag,
            qubit_mapping=init_mapping,
            qubit_mask=qubit_mask_,
            front_layer=front_layer_,
        )

        self._cur_node = MCTSNode(
            circuit_dag=self._circuit_dag,
            coupling_graph=self._coupling_graph,
            front_layer=self._circuit_dag.front_layer,
            qubit_mask=qubit_mask.copy(),
        ...
        for swap_label in swap_label_list:
            ...
            self._physical_circuit.append(
                self._coupling_graph.get_swap_gate(swap_label)
            )
            self._add_executable_gates(node=self._cur_node)
        ...
        if self._is_generate_data:
            (
                ...
                value,
                action_prob,
            ) = self._mcts_wrapper.get_data()
            self._experience_pool.extend(
                adj=adj,
                qubits=qubits,
                action_probability=action_prob,
                value=value,
                circuit_size=padding_mask,
                swap_label=swap_gate,
                num=num,
            )
    ...
    def _add_initial_single_qubit_gate(self, node: MCTSNode):
        ...
        while len(sqg_stack) > 0:
            ...
            if self._logical_circuit_dag[gate]["gate"].is_single():
                self._physical_circuit.append(
                    self._get_physical_gate(
                        gate=self._logical_circuit_dag[gate]["gate"],
    ...
    def _add_executable_gates(self, node: MCTSNode):
        ...
        self._num_of_executable_gate = self._num_of_executable_gate + len(
            execution_list
        )
        ...
        for gate in execution_list:
            if self._logical_circuit_dag[gate]["gate"].is_single():
                raise Exception(
                    "There shouldn't exist single qubit gate in two-qubit gate circuit"
                )
            else:
    ...
    def _add_gate_and_successors(self, gate: int):
        ...
        while len(sqg_stack) > 0:
            ...
            self._physical_circuit.append(
                self._get_physical_gate(
                    gate=self._logical_circuit_dag[gate_t]["gate"],
                    cur_mapping=cur_mapping,
                )
            )
    ...
    def _is_valid_successor(
        self, gate_index: int, pre_gate_index: int, mark: Set[int]
    ) -> bool:
        """
