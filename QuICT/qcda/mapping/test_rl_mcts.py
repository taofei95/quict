...
from torch.multiprocessing import Pool, Process, Value, Pipe, Queue, SimpleQueue ,set_start_method
...
class AlphaQut(object):
    ...
    def __init__(self, graph_name: str, config: GNNConfig, log_path: str = None, train_log_path: str = None, train_tb_path: str = None, mcts_log_path: str = None, model_path: str = None):
        ...
        self._experience_pool = ExperiencePool(max_capacity = config.maximum_capacity, num_of_nodes = config.num_of_nodes, num_of_class = self._coupling_graph.num_of_edge) 
    ...
    def __call__(self, num_of_iterations: int = 5, num_of_circuits: int = 100, input_path: str = None):
        ...
        idx = Value('i')
        cur_model_path = self._model_path
        q = Queue()
        send_conn = []
        recv_conn = []
        ...
        for i in range(num_of_iterations):
            ...
            cur_model_path = f"{self._model_path}_{i}"
            copyfile(pre_model_path, cur_model_path)
            idx.value = 0
            start_time = time.time()
            seed = prg.integers(low = 0, high = 10000, size = num_of_circuits)
            p_inference = Process(target = inference_process, args = (self._graph_name, self._config, cur_model_path, q, send_conn))
            ...
            with Pool(processes = self._config.num_of_process, initializer = init, initargs = (self._experience_pool, q, recv_conn, idx)) as pool:
                ...
                for i in range(num_of_circuits):
                    pool.apply_async(mcts_process, 
                                args = (init_mapping, self._graph_name, self._config.minimum_circuit, self._config.maximum_circuit, 5, self._coupling_graph.size,  seed[i], self._mcts_log_path, i, cur_model_path, self._config),
            ...
            p_train = Process(target = trainer_process, args = (self._graph_name, self._config, self._train_log_path, self._train_tb_path, cur_model_path, input_path))
            p_train.start()
            p_train.join()
            p_train.close()
