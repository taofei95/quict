...
int main(int argc, char* argv[]){
    ...
    int info, method, repeat_times =1, major = 1;
    ...
    info = std::stoi(std::string(argv[14])); 
    method = std::stoi(std::string(argv[15]));
    repeat_times = std::stoi(std::string(argv[16])); 
    major = std::stoi(std::string(argv[17]));
    mcts::MCTSTree mcts(major, method, info, extended, with_predictor, 
    ...
    for(int i =0;i < repeat_times;i++){
        mcts.load_data(num_of_gates, num_of_qubits, circuit, dependency_graph);
        mcts.build_search_tree(qubit_mapping, qubit_mask, front_layer);
        std::vector<int> res = mcts.run();
