...
cdef extern from "data_generator.h" namespace "mcts":
    cdef cppclass DataGenerator:
        DataGenerator(int major, int info, bool with_predictor, bool is_generate_data, int threshold_of_circuit, float gamma, float c, float virtual_loss, int bp_mode, 
                int num_of_process, int size_of_subcircuits, int num_of_swap_gates, int num_of_iterations, int num_of_playout,
                ...
                const char* model_file_path, int device, int num_of_circuit_process, int inference_batch_size, int max_batch_size)
