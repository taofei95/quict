...
class ForwardMatch:
        ...
        for node_id in pred:
            for dir_succ in self.template_dag_dep.direct_successors(node_id):
                if dir_succ not in matches:
                    ...
                    block = block + succ
        self.candidates = list(set(self.template_dag_dep.direct_successors(node_id_t))
    ...
    def _init_matched_nodes(self):
        ...
        """
        self.matched_nodes_list.append([self.node_id_c,
                                        self.circuit_dag_dep.get_node(self.node_id_c)])
    ...
    def _is_same_op(self, node_circuit, node_template):
        ...
        """
        return node_circuit.name == node_template.name
    ...
    def _is_same_q_conf(self, node_circuit, node_template):
        ...
        if node_circuit.gate.is_single():
            return self.qarg_indices == node_template.qargs
        # CX, CCX
        else:
            c_template = node_template.gate.controls

            if c_template == 1:
                return self.qarg_indices == node_template.qargs

            else:
                control_qubits_template = node_template.qargs[:c_template]
                ...
                if set(control_qubits_circuit) == set(control_qubits_template):
                    ...
                    target_qubits_template = node_template.qargs[c_template::]
    ...
    def run_forward_match(self):
        ...
        while self.matched_nodes_list:
            ...
            qarg1 = self.circuit_dag_dep.get_node(label).qargs

            # Update the indices for qubits in order to be comparable with the
            ...
            for i in self.candidates:
                ...
                if len(self.qarg_indices) != len(node_template.qargs) \
                        or set(self.qarg_indices) != set(node_template.qargs)\
                        or node_circuit.name != node_template.name:
                    continue

                # Check if the qubit configuration are compatible for a match,
            ...
            if not match:
                ...
                for succ in v[1].successors:
                    ...
                    if self.circuit_dag_dep.get_node(succ).matchedwith:
                        self.match.remove([self.circuit_dag_dep.get_node(succ).matchedwith[0],
                                           succ])
