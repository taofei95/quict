...
# search for S, CNOT, H gates
    ...
    def copy_circuit(self, temp_circuit):
        new_circuit = Circuit(temp_circuit.circuit_width())
        new_circuit.id = temp_circuit.id
        new_circuit.const_lock = temp_circuit.const_lock
        new_circuit.name = temp_circuit.name
        for i in temp_circuit.gates:
            new_circuit.gates.append(i)
        for i in temp_circuit.topology:
            new_circuit.topology.append(i)
        return new_circuit
    ...
    def search(self, temp_gate_num, temp_circuit):
        ...
        for s in range(0, self.qubit_num):
            if self.temp_qubit_dep[s] + 1 <= self.dep:
                ...
                temp_result = self.search(temp_gate_num + 1, new_circuit)
                # self.temp_qubit_dep[s] -= 1
            ...
            for cnot_b in range(0, self.qubit_num):
                if cnot_a != cnot_b:
                    ...
                    if temp_max_dep <= self.dep:
                        new_circuit = copy.deepcopy(temp_circuit)
                        ...
                        # self.temp_qubit_dep[cnot_a] = temp_dep_b

        return temp_result

    def run_template_searching(self):
        circuit_temp = Circuit(self.qubit_num)
        result = self.search(0, circuit_temp)
        return self.template_list

program = TemplateSearching(1, 8, 8)
