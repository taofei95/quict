...
class RandomWalk:
    ...
    def _graph_validation(self):
        ...
        self._position_qubits = int(np.log2(self._position_space))
    ...
    def _cop_validation(self):
        ...
        self._coin_qubits = int(np.log2(shape[0]))

    def _circuit_construct(self):
        self._circuit = Circuit(self._position_qubits + self._coin_qubits)
        action_gate = Unitary(self._coin_operator)
        action_qubits = [self._position_qubits + i for i in range(self._coin_qubits)]
    ...
    def _build_shift_operator(self):
        unitary_matrix = np.zeros(
            (self._position_space * self._action_space, self._position_space * self._action_space),
            dtype = np.complex128
        )
        for i in range(self._position_space):
            curr_idx = self._action_space * i
