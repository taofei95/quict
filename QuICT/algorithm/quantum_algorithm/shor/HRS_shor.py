...
def shor(N):
    ...
    for b in range(2, L):
        ...
        if pow(u1, b) == N:
            print(
                'Shor succeed: N is exponential, found the only factor %d classically' % u1)
            return u1
        if pow(u2, b) == N:
            print(
                'Shor succeed: N is exponential, found the only factor %d classically' % u2)
            return u2
    ...
    while rd < max_rd:
        ...
        if gcd > 1:
            print(
                'Shor succeed: randomly chosen a = %d, who has common factor %d with N classically' % (a, gcd))
            return gcd
        ...
        if r == 0:
            ...
            if r % 2 == 1:
                ...
                if h == N - 1:
                    print(
                        'Shor failed: found order r = %d of a = %d with negative square root' % (r, a))
                else:
                    f1 = np.gcd(h - 1, N)
                    f2 = np.gcd(h + 1, N)
                    if f1 > 1 and f1 < N:
                        print(
                            'Shor succeed: found factor %d, with the help of a = %d, r = %d' % (f1, a, r))
                        return f1
                    elif f2 > 1 and f2 < N:
                        print(
                            'Shor succeed: found factor %d, with the help of a = %d, r = %d' % (f2, a, r))
                        return f2
                    else:
