...
class CNFSATOracle:
    ...
    def run(
        ...
        if clause_number==1:
            ...
            one_dirty_aux(self._cgate, controls, target, current_Aux) #QuICT.qcda.synthesis.mct.
        else:
            ...
            one_dirty_aux(self._cgate, controls, variable_nunmber, current_Aux)
            for j in range(block_number):
                self.clause(
                    CNF_data, variable_nunmber,
                    ancilla_qubits_num,  j * block_len, np.minimum((j+1) * block_len-1, clause_number),
...
if __name__=="__main__":
    cnf = CNFSATOracle()
    cnf.run("./QuICT/algorithm/quantum_algorithm/CNF/1.cnf")
    cgate = cnf.circuit()
    print(cgate.qasm())
