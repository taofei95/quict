...
class CNFSATDEPTHOracle:
    ...
    def run(
        ...
        assert ancilla_qubits_num > 3 * clause_length + 2 , "Need more auxiliary qubit or some CNF-clause contains too many variables.."
    ...
    def clause(self, CNF_data: List, variable_number: int, Aux: int, clause_length :int, CleanQubitNumber  :int, StartID: int, EndID: int, target: int, current_depth: int, depth: int):
        ...
        if StartID == EndID: 
            ...
            if ((EndID - StartID) * (clause_length) < (Aux - 2 * CleanQubitNumber + 1)) & ((EndID - StartID ) < p) : #if block_number == 1 而且 EndID - StartID > 1 
                ...
                #print(controls_abs)
                #print(target)
                MCTOneAux().execute(len(controls_abs) + 2) | self._cgate(controls_abs0 + [ c[0]+ CleanQubitNumber, c[0]  ])
                # one_dirty_aux(self._cgate, controls_abs, target, current_Aux)
                #X | self._cgate(target)
                for i in range(len(controls_X0)):
                    X | self._cgate(controls_X0[i])
