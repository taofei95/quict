...
def test():
    ...
    for i in i_list:
        for j in j_list:
            ...
            cgate = cnf.circuit()
            # print(f"{j}_{i}_5 size:{cgate.size()}")
            # print(f"{j}_{i}_5 depth:{cgate.depth()}")
            
            import pandas as pd
            df = pd.read_excel('QuICT/algorithm/quantum_algorithm/CNF/test_data/data.xlsx')
            data = df.head()
            print(f"{j}_{i}_5 size:{cgate.size()}".format(data))
            print(f"{j}_{i}_5 depth:{cgate.depth()}".format(data))
