...
def _cos_estimate(
    ...
    for _ in range(m):
        oracle.S_chi(n, controlled=True) | circ([n+n_ancilla]+list(range(n+n_ancilla))) # assert default control bit on 0
        state_preparation.A_dagger(n)    | circ(list(range(n+n_ancilla)))
        Ry(-asin(1/4)*2)                 | circ([n+n_ancilla])
        state_preparation.S_0(n+1)       | circ([n+n_ancilla]+list(range(n+n_ancilla)))
        state_preparation.A(n)           | circ(list(range(n+n_ancilla)))
        Ry( asin(1/4)*2)                 | circ([n+n_ancilla])
    # for j in range(n):
    #     Measure | circ(j)
...
def amplitude_estimate(
    ...
    for j in range(1,l+1):
        if is_first_stage:
            c_estimate = _cos_estimate(2**(j-1),N_1_shot,oracle,state_preparation,simulator)
            c_min, c_max = _chernoff(c_estimate,N_1_shot,delta_c)
            theta_min,theta_max = acos(c_max)/(2**(j+1)+2),acos(c_min)/(2**(j+1)+2) # update
            if (2**(j+1))*theta_max>=3*pi/8 and j<l:
                j0=j
                mu=(2**j0)*(theta_min+theta_max)
                is_first_stage = False
        else:
            c_estimate = _cos_estimate(2**(j-1),N_2_shot,oracle,state_preparation,simulator)
            s_estimate = (c_estimate*cos(mu) - _cos_estimate(2**(j-1)+2**(j0-1),N_2_shot,oracle,state_preparation,simulator))/sin(mu)
            rmd_by_2pi = _atan(s_estimate, c_estimate)
            quo_by_2pi = floor(((2**(j+1)+2)*theta_max-rmd_by_2pi+pi/3)/(2*pi))
            theta_min,theta_max = (2*pi*quo_by_2pi+rmd_by_2pi-pi/3)/(2**(j+1)+2),(2*pi*quo_by_2pi+rmd_by_2pi+pi/3)/(2**(j+1)+2) #update
