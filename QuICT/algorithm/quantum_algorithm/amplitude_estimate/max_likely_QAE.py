...
def amplitude_estimate(
    ...
    """maximum likelyhood estimation. The list of (N,m) is constructed from eps in EIS style where N_shot=100
    ...
    def loglikelyhood(theta):
        ...
        sec_half = (all_counts-good_counts) * np.log(np.cos((list_m*2+1)*theta)**2)
        return -(np.sum(fst_half+sec_half))
    est_theta = brute(loglikelyhood, [[0+search_eps, np.pi/2-search_eps]])[0]
