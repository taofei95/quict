...
class RCOutOfPlaceModMultiplier(CompositeGate):
    """
        ...
        Montgomery Reduction. Uses in total "2n + logn + 1" qubits which
        includes "logn + 1" clean ancilla qubits.
    ...
    def __init__(
        ...
        """
            ...
            Gate needs width: `2 * n + ceil(log2(n)) + 1`.

            Args:
                modulus (int):
                    modulus in the modular multiplication.

                multiple (int):
                    multiple in the modular multiplication.

                qreg_size (int):
                    the number of qubits to hold the input register and
                    store the result, two sizes are the same.

                inverse_multiple (bool):
                    if True, will calculate "M^(-1)*z mod N".
    ...
    def _phi_MAC_list(
        ...
        # general cases, no depth optimization
            ...
            for i in range(in_reg_size):
                # apply each adder gate controlled by bits on the input
                # register from the lowest to the highest bit
...
class RCModMultiplier(CompositeGate):
    """
        ...
        Montgomery Reduction. Uses in total "2n + logn + 1" qubits which
        includes "n + logn + 1" clean ancilla qubits.
    ...
    def __init__(
        ...
        """
            ...
            NOTE: currently the quantum data inside |z(n)> has to be smaller
            than modulus for the circuit to properly set all the ancilla qubits
            back to 0s. This is due to the use of out-of-place modular
            multiplier with inversed multiple. When running the circuit
            backwards, it cannot cancel z >= modulus (it is because when
        ...
        # *multiple^(-1) % modulus, backwards(inverse)
        RCOutOfPlaceModMultiplier(
            modulus=modulus,
            multiple=multiple,
            qreg_size=qreg_size,
...
class RCModMultiplierCtl(CompositeGate):
    """
        A controlled in-place modular multiplier using Fourier-basis
        arithmetic and Montgomery Reduction. Uses in total "2n + logn + 1"
        qubits which includes "n + logn + 1" clean ancilla qubits.
