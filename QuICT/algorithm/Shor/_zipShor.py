...
def Shor(N, fidelity = None):
    ...
    while True:
        ...
        if N % c == 0 and c != 1 and N != b:
            return c, a, r, total_gate_number / rd, total_run_time / rd, total_gate_number, total_run_time, total_IQFT_gate / rd, total_IQFT_gate, total_oracle_gate_number / rd, total_oracle_gate_number, rd, []

class zip_shor_factoring(Algorithm):
    @staticmethod
...
if __name__ == "__main__":
    time_start = time.time_ns()
    zip_shor_factoring.run(1019 * 1021)
    time_end = time.time_ns()
