...
class qconv2:
    ...
    def __call__(self,wires:list,param:Variable) -> CompositeGate:
        """
        Args:
...
class pool:
    ...
    def __call__(self,param:Variable,wires:list) -> Trigger:
        """
        Args:
...
class FC:
    ...
    def __call__(self,G_param:Variable,G2_param:Variable,wires:list)-> CompositeGate:
        """
        Args:
            G_param(Variable): the param of U3gates,it is expected to be a  Variable with _*3 size
            G2_param(Variable): the param of CU3gates,it is expected to be a  Variable with _*3 size
            reference:
            https://journals.aps.org/pra/abstract/10.1103/PhysRevA.101.032308
        """
        ...
        for i in range(len(wires)):
            G_gate = U3
            G_gate = G_gate(G_param[i],G_param[i+1],G_param[i+2])
            G_gate & wires[i] | fc_gate
        l= len(wires)
        for i in range(l):
            cu3_gate = CU3
            cu3_gate = cu3_gate(G2_param[i],G2_param[i+1],G2_param[i+2])
...
class QCNNLayer(QNNLayer):
    ...
    def circuit_layer(self,  params,qconv:qconv2,pool:pool,wires:list,com_gate:CompositeGate):
        ...
        for i in range(1,n_wires,2):  # one layer contains two conv and one pool
            ...
            qconv(wires=wire_list1,param= params[0,index_param0:index_param0+4]) |com_gate
            index_param0=index_param0+4
            qconv(wires=wire_list2,param= params[0,index_param0:index_param0+4]) |com_gate
            index_param0=index_param0+4
            pool(param=params[1,index_param1:index_param1+6],wires=wire_list2)|com_gate
