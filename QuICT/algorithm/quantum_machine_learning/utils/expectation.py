...
class Expectation(tf.keras.layers.Layer):
    ...
    def call(self, inputs, *args, **kwargs):
        ...
        ket = self.sv.copy()
        e_val =np.dot( np.dot(bra, self._operators.get_hamiton_matrix(n_qubits=self._model_circuit.width())) , ket).real
        #e_val = np.dot(e_val, ket).real
        return np.dot( np.dot(bra, self._operators.get_hamiton_matrix(n_qubits=self._model_circuit.width())) , ket).real