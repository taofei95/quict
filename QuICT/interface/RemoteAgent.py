...
class QuantumLeafAgent(RemoteAgent):
    ...
    def Execute(self, jobid):
        log_.info('QuantumLeaf Start Execute...')
        ...
        if cb ==True:
            ...
            log_.info('QuantumLeaf Execute Successfuly...')
            return result
        else:
            log_.error('Execute State is worry')
            return 
...
class QiskitAgent(RemoteAgent):
    ...
    def Execute(self, jobid):
        log_.info('Qiskit Start Execute...')
        cb = self.AgentChangeExeState(jobid,0)
        if cb ==True:
            context= self.AgentGetJob(jobid)
            ...
            log_.info('Qiskit Execute Successfuly...')
