...
class DiagonalGate(object):
    ...
    def __init__(self, target: int, aux: int = 0):
        ...
        self.target = target
        if np.mod(aux, 2) != 0:
            self._logger.warn('Algorithm serves for even '
                              'number of auxiliary qubits.'
                              ' One auxiliary qubit is dropped.')
            aux = aux - 1
        self.aux = aux
    ...
    def __call__(self, theta):
        """
        Args:
            theta(listLike): list of (2 ** target) angles
            of rotation in the diagonal gate
    ...
    def no_aux_qubit(self, theta):
        """
        Args:
            theta(listLike): list of (2 ** target)
            angles of rotation in the diagonal gate
    ...
    def lucal_gray_code(k, n):
        """
        Generate the (k, n)-Gray
        code defined in
        and following Lemma 7

        Args:
            k(int): start the circular modification
            from the k-th binary code
            n(int): the length of binary code, that is,
             the length of Gray code would be 2^n
    ...
    def binary_inner_prod(s, x, width):
        ...
        where s_bin and x_bin are binary representation of
        s and x respectively of width n
    ...
    def phase_shift(cls, theta, seq=None, aux=None):
        ...
        Args:
            theta(listLike): phase angles of the diagonal gate
            seq(iterable, optional): sequence of s application,
             numerical order if not assigned
    ...
    def phase_shift_s(cls, s, n, alpha, aux=None, j=None):
        """
        Implement the phase shift for a certain s
        defined in Equation 5 as Figure 8
        ...
        Args:
            ...
            aux(int, optional): key of auxiliary qubit (if exists)
            j(int, optional): if no auxiliary qubit, the j-th
            smallest element in s_idx would be the target qubit
        ...
        if aux is not None:
            if j is not None:
                cls._logger.warn('With auxiliary qubit in phase_shift_s, '
                                 'no i_j is needed.')
            assert aux >= n, ValueError('Invalid auxiliary qubit '
                                        'in phase_shift_s.')
            for i in s_idx:
                CX & [i, aux] | gates
            U1(alpha) & aux | gates
        ...
        # Figure 8 (b)
            ...
            assert j < len(s_idx), ValueError('Invalid target in phase_shift'
                                              ' without auxiliary qubit.')
            for i in s_idx:
                if i == s_idx[j]:
                    continue
                CX & [i, s_idx[j]] | gates
