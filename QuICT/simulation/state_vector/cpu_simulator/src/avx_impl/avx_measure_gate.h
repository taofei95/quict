...
#ifndef SIM_BACK_AVX_MEASURE_GATEH
#define SIM_BACK_AVX_MEASURE_GATEH
...
int MaTricksSimulator<Precision>::apply_measure_gate(uint64_t q_state_bit_num,
...
#pragma omp for
      ...
      __m256d ymm0 = chosen ? _mm256_set_pd(1. / coef_1, 0, 1. / coef_1, 0)
                            : _mm256_set_pd(0, 1. / coef_0, 0, 1. / coef_0);

#pragma omp parallel for if (q_state_bit_num > sysconfig_.omp_threshold_) \
    num_threads(sysconfig_.omp_num_thread_)
      for (uint64_t i = 0; i < (1 << q_state_bit_num); i += batch_size) {
        __m256d ymm1 = _mm256_loadu_pd(&real[i]);
        __m256d ymm2 = _mm256_loadu_pd(&imag[i]);
...
#pragma omp for
        ...
        _mm256_storeu_pd(ymm0_unpack, ymm0);
        amp_sum +=
            ymm0_unpack[0] + ymm0_unpack[1] + ymm0_unpack[2] + ymm0_unpack[3];
      }

      int chosen = random_dist(random_gen) > amp_sum;
      double coef_0 = sqrt(amp_sum), coef_1 = sqrt(1. - amp_sum);
      __m256d ymm0 = chosen ? _mm256_set_pd(1. / coef_1, 1. / coef_1, 0, 0)
...
#endif  // SIM_BACK_AVX_MEASURE_GATEH
