...
#ifndef SIM_BACK_AVX_X_GATEH
#define SIM_BACK_AVX_X_GATEH
...
void MaTricksSimulator<Precision>::apply_x_gate(uint64_t q_state_bit_num,
...
#pragma omp parallel for num_threads(omp_thread_num) \
    schedule(dynamic, omp_chunk_size(q_state_bit_num))
      for (uint64_t ind = 0; ind < (1ULL << q_state_bit_num);
           ind += batch_size) {
