...
def test_unitary_matrix_encoding_lcu():
    ...
    G, G_inverse, unitary_encoding, unitary_encoding_inverse = UME.execute(coefficient_array, matrix_array, complete=False, phase_gate=True)
    ...
    expected_matrix = np.matmul(np.matmul(np.linalg.inv(expected_B_matrix), expected_control_v_matrix), expected_B_matrix)
...
def test_unitary_matrix_encoding_conj():
    ...
    circuit_matrix = circuit.matrix()
    circuit_matrix = np.array([[circuit_matrix[0][0], circuit_matrix[0][1]],[circuit_matrix[1][0], circuit_matrix[1][1]]])
    expected_matrix = np.array([[0.2,0.3],[0.3,-0.2]])
    assert np.allclose(expected_matrix, circuit_matrix), "Circuit matrix doesn't true."
test_unitary_matrix_encoding_lcu()
test_unitary_matrix_encoding_conj()
