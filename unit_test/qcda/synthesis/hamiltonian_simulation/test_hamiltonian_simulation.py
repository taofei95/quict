...
def test_hamiltonian_simulation():
    ...
    assert (np.sum(np.abs(final_state-circuit_dictionary["approximated_time_evolution_operator"][:,0])) < 0.1), "TS method is not correct."
    ...
    circuit, _ = HS.execute(hamiltonian=[[1, 'X0', 'Y1'], [1, 'Z0', 'X1']],
                            ...
                            error=0.05)
    final_state = trotter.accurate_final_state([[1, 'X0', 'Y1'], [1, 'Z0', 'X1']], 1, init_statevec=np.array([0, 0, 0, 1]))
