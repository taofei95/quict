            ...
            for j in range(len(CNF_data[i+1])):
                if CNF_data[i+1][j] > 0:
                    clause_result = clause_result + \
                        variable_data[CNF_data[i+1][j]-1]
                else:
                    if CNF_data[i+1][j] < 0:
                        clause_result = clause_result + \
...
def one_test(filename_test, variable_number, clause_number, CNF_data, n_solution, runs):
    ...
    circ = grover.circuit(variable_number, AuxQubitNumber + 1,
                          oracle, n_solution, measure=False, is_bit_flip=True)
    grover.simulator.run(circ)
    result_samples = grover.simulator.sample(runs)
    result_var_samples = np.array(result_samples).reshape(
        (1 << variable_number, 1 << (AuxQubitNumber + 1))).sum(axis=1)
...
def test_cnf():
    ...
    for filename_test in filename_test_list:
        ...
        if n_solution == 0:
            ...
            n_hit = one_test(file_path, variable_number,
                             clause_number, CNF_data, n_solution, n_all)
            print(
            ...
            assert abs(n_hit/n_all - sin((2*n_iter+1)*theta)
                       ** 2) < 1/sqrt(n_all)
