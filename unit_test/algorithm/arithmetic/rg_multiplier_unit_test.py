...
from QuICT.algorithm.arithmetic.multiplier import RGMultiplier


class TestRGMultiplier(unittest.TestCase):
    ...
    def test_single_correctness(self):
        ...
        for count in decoded_counts:
            input_a, input_b, output_axb = count
            # check correctness
    ...
    def _construct_multi_cric(
        ...
        multi_circ = Circuit(2 * (reg_size1 + reg_size2))
        ...
        # apply multiplier
        RGMultiplier(reg_size1, reg_size2) | multi_circ
    ...
    def _run_and_decode(
        ...
        counts = sv_sim.sample(shots=(1 << (reg_size1 + reg_size2 + 2)))

        return decode_counts_int(counts, [reg_size1, reg_size2, reg_size1 + reg_size2])
