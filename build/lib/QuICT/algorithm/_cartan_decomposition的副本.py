...
def SU2_to_U3(P, qubit):
    if abs(P[0, 0].real) < 1e-6:
        if abs(P[0, 0].imag) > 1e-6:
            P[:] *= 1j
    else:
        alpha = -np.arctan(P[0, 0].imag / P[0, 0].real)
        phase = np.exp(alpha * 1j)
        P[:] *= phase
    tha = np.arccos(P[0, 0]) * 2
    if abs(tha) < 1e-6:
        thb = 0
        thc = np.angle(P[1, 1])
    else:
        thc = np.angle(P[0, 1] / -np.sin(tha / 2))
        thb = np.angle(P[1, 0] / np.sin(tha / 2))
...
def NKS_2qubit(low, high, U : np.matrix):
    ...
    # 4 compute D^(1/2) Kp
    P = np.mat(np.zeros((4, 4)), dtype=complex)
    for i in range(4):
        P[:, i] = angle[i][1]
    ...
    assert np.all(abs(P * Dd2 * Dd2 * P.H - M2) < 1e-6)
    ...
    thx = (angle[0][0] + angle[2][0]) / 2
    thy = (angle[1][0] + angle[2][0]) / 2
    thz = (angle[0][0] + angle[1][0]) / 2
    ...
    gates.extend([SU2_to_U3(VA, low), SU2_to_U3(VB, high - 1)])
    return gates
...
if __name__ == '__main__':
    ...
    back_circuit = Cartan_decomposition.run(SyntheticalUnitary.run(test_circuit))
    back_circuit.print_infomation()
    unitary = SyntheticalUnitary.run(back_circuit)
    print(np.round(unitary, decimals=2))
