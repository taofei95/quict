#!/usr/bin/env python
# -*- coding:utf8 -*-
# @TIME    : 2020/3/3 12:03 上午
# @Author  : Han Yu
# @File    : _alter_depth_decomposition.py
...
def gate2perm(gate : BasicGate, n_size):
    ...
    if gate.type() == GateType.Perm:
        ...
        for i in range(1 << (n_size - 2)):
            if bincaat(i, n_size - 1 - gate.cargs[0], n_size - 1 - gate.cargs[1], 0, 0) & (1 << (n_size - 1 - gate.targs)) != 0:
                temp1 = bincaat(i, n_size - 1 - gate.cargs[0], n_size - 1 - gate.cargs[1], 1, 1) ^ (1 << (n_size - 1 - gate.targs))
...
class ALTER_DEPTH_DECOMPOSITION(circuit2circuit):
    @staticmethod
