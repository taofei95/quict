...
from QuICT.ops.linalg.cpu_calculator import matrix_dot_vector
...
class TestGPULinalg(unittest.TestCase):
    ...
    def test_matrixpermutation(self):
        mapping = list(range(TestCPULinalg.seed))[::-1]
        mapping = np.array(mapping)
        np.random.shuffle(mapping)

        gpu_result = GPUCalculator.MatrixPermutation(TestGPULinalg.matrix_A, mapping)
        self.assertTrue(np.isclose(np.sum(gpu_result), np.sum(TestGPULinalg.matrix_A), atol=1e-04))
...
class TestCPULinalg(unittest.TestCase):
    ...
    def test_matrix_dot_vector_cpu(self):
        np_result = np.kron(TestCPULinalg.matrix_A, TestCPULinalg.matrix_B)
