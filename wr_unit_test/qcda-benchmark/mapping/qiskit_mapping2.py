    ...
    for q_num in qubits_num:
        ...
        for gm in gate_multiply:
            ...
            for i in range(1, 6):
                ...
                lines_mapping = [[0,1],[1,2],[2,3],[3,4]]
                
                cir_q = QuantumCircuit.from_qasm_file(circuit_folder_path + '/' + file_name)
                coupling_map = CouplingMap(couplinglist=lines_mapping)
                ss = BIPMapping(coupling_map=coupling_map)
                pass_manager = PassManager(ss)
                bipmap_circ = pass_manager.run(cir_q)
                f.write(f"{i}, Qiskit swap gates number: {ss.max_swaps_inbetween_layers()}\n")
