...
# x0 x1，x2, x_{n variable_nunmber -1}
    filename_test =  "./2.cnf"
    variable_nunmber , clause_number , CNF_data = read_CNF(filename_test)


    #真值表初值变化
    for a in range(2 ** variable_nunmber):  #改
        cnf = CNFSATOracle()
        cnf.run(filename_test)
        cgate = cnf.circuit()
        circuit_temp=Circuit(variable_nunmber + 2 + 4)
        randomnum = a
        x=[]
        for ii in range(variable_nunmber):
        ...
        print(int(circuit_temp.qubits[variable_nunmber])) #量子结果
        ...
        assert int(circuit_temp.qubits[variable_nunmber] and cnf_result
test()