...
bin_img = np.random.choice(2, 64).reshape((8, 8))
frqi = FRQI(2)
start = time.time()
frqi_circuit_qic = frqi(bin_img, use_qic=True)
print("Construct FRQI with QIC: ", time.time() - start)
start = time.time()
frqi_circuit_noqic = frqi(bin_img, use_qic=False)
print("Construct FRQI without QIC: ", time.time() - start)
...
print("Construct NEQR with QIC: ", time.time() - start)
start = time.time()
neqr_circuit_noqic = neqr(bin_img, use_qic=False)
print("Construct NEQR without QIC: ", time.time() - start)
neqr_qic = simulator.run(neqr_circuit_qic)
neqr_noqic = simulator.run(neqr_circuit_noqic)
print(
    "NEQR with and without QIC should be same: ",
    abs(np.linalg.norm(neqr_qic - neqr_noqic)) < 1e-12,
)
print(
    "NEQR and FRQI for binary image should be same: ",
    abs(np.linalg.norm(frqi_noqic - neqr_noqic)) < 1e-12,
)


print("---------------------------Test Grayscale Image-------------------------------")

gray_img = np.random.choice(256, 64).reshape((8, 8))
frqi = FRQI(256)
start = time.time()
...
frqi_noqic = simulator.run(frqi_circuit_noqic)
print(
    "FRQI with and without QIC should be same: ",
    abs(np.linalg.norm(frqi_qic - frqi_noqic)) < 1e-12,
)
